...
impl<'a> L3GD20<'a>
{
    ...
    pub fn read_temp(&self) -> Result<i8> {
        let mut data = [0];
    ...
    /// Configure `CTRL_REG1`
    ///
    /// Assumes user wants to enable sensor on all axis
    ...
    fn config_reg3(&self, interrupt: bool) -> Result<()> {
        // TODO: Add support for interrupt on PIN 2
        let cmd = if interrupt { 1 << 7 } else { 0 };
